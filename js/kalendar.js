...
/*************    方法     **************/
	...
	function kalendarCode(codeYear,codeMonth,codeDay,codeFirst){
		kalendar_html = "<table id='kalendar' cellpadding='0' cellspacing='0'>\n<tr id='select'>\n<td colspan=7>\n<div id='year'>\n<ul>\n<li><input type='button' id='yearPrev' value='<' /></li>\n<li class='selectChange'>\n<select name='year'>";
		//年 选择
		for(var i=today['year'];i<=codeYear+yearfloor;i++){
			if(i == codeYear){
...
/*************    缓存节点和变量     **************/
	...
	var yearfloor = 60;//选择年份从1970到当前时间的后10年
...
/***************************/
	...
	yearPrev.bind('click',function(){
		yearChange --;
		if(yearChange < today['year']){
			yearChange = today['year'];
			/*alert('太前也没意义了...');*/
			return false;
		}
		//修改年份
	...
	$('.dayList>li').click(function(){
		$(this).addClass('pb_current').siblings().removeClass('pb_current');
		$(this).parent().siblings().children().removeClass('pb_current');
	});
